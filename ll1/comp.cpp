...
int main(int argc, char const *argv[])
{
	...
	while(std::getline(grammar_file, buffer)) {
		...
		for(auto i = vstrings.begin(); i!= vstrings.end();i++) {
			if((*i).substr(0,2)=="t_") cout<<(*i).substr(2, (*i).length()-2)<<" ";
			else cout<<*i<<" ";
		}
			cout<<endl;
	}
	...
	for(auto it = firsts.begin(); it != firsts.end(); ++it) {
		...
		for(auto firsts_it = it->second.begin(); firsts_it != it->second.end(); ++firsts_it) {
			...
			if((*firsts_it).substr(0,2)=="t_") cout<<(*firsts_it).substr(2, (*firsts_it).length()-2)<<" ";
			else cout<<*firsts_it<<" ";
	...
	for(auto it = follows.begin(); it != follows.end(); ++it) {
		...
		for(auto follows_it = it->second.begin(); follows_it != it->second.end(); ++follows_it) {
			...
			if((*follows_it).substr(0,2)=="t_") cout<<(*follows_it).substr(2, (*follows_it).length()-2)<<" ";
			else cout<<*follows_it<<" ";
			//cout<<*follows_it<<" ";
		}
		cout<<"\n";
	}
...
for(auto i = terms.begin(); i != terms.end(); ++i) {
	...
	while(std::getline(input_file, input_string_r, ' '))
	{
	   v_input_string.push_back(input_string_r);
	}
	if(v_input_string.size() == 0) 
	{
...
*/
	...
	for(auto ch = v_input_string.begin(); ch != v_input_string.end(); ++ch) {
		if((*ch)!="$" &&terms.find("t_"+*ch) == terms.end()) {
	...
	while(!st.empty() && !v_input_string.empty()) {
		...
		if("t_"+v_input_string[0] == st.top() || v_input_string[0] == st.top() ) {
			st.pop();
			...
			string inp = v_input_string[0];
			if(inp != "$") inp = "t_"+inp;
			int col = distance(terms.begin(), terms.find(inp));
			...
			if((*ch).substr(0,2)=="t_") cout<<(*ch).substr(2, (*ch).length()-2)<<" ";
			else cout<<*ch<<" ";
				//cout<<*ch<<" ";
				st.push(*ch);
			}
